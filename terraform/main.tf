...
resource "google_firestore_document" "admin_list" {
  count = data.external.firestore_check.result["exists"] == "true" ? 0 : 1
  project     = var.project_id
  database    = "(default)"
  collection  = "settings"
  document_id = "adminList"

  fields = "{ \"emails\": { \"array_value\": { \"values\": [{ \"string_value\":\"email1@example.com\"}, {\"string_value\":\"m.barsukou@gmail.com\"}]}}}"
